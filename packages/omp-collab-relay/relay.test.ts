/**
 * Contract tests mirror OMP v17.2.6's local relay. Hardening tests defend the
...
function unpackEnvelope(
	...
	const peerId = new DataView(data.buffer, data.byteOffset, 4).getUint32(
		0,
		false,
	);
...
describe("omp-collab-relay contract", () => {
	it("serves health and validates relay requests", async () => {
		...
		expect((await fetch(`${relayHttpUrl()}/r/${ROOM}?role=admin`)).status).toBe(
			404,
		);
	...
	it("requires Cloudflare Access for web and guest traffic while admitting the local native host", async () => {
		...
		expect((await fetch(`${relayHttpUrl()}/api/rooms`)).status).toBe(401);
		expect((await fetch(`${relayHttpUrl()}/r/${ROOM}?role=guest`)).status).toBe(
			401,
		);
		expect((await fetch(`${relayHttpUrl()}/r/${ROOM}?role=host`)).status).toBe(
			426,
		);
		...
		expect(
			(await fetch(`${relayHttpUrl()}/`, { headers: accessHeaders })).status,
		).toBe(200);
		expect(
			(await fetch(`${relayHttpUrl()}/api/rooms`, { headers: accessHeaders }))
				.status,
		).toBe(200);
		expect(
			(
				await fetch(`${relayHttpUrl()}/r/${ROOM}?role=guest`, {
	...
	it("serves the same-origin browser client with hardened headers", async () => {
		...
		expect(index.headers.get("cross-origin-resource-policy")).toBe(
			"same-origin",
		);
		...
		expect(scriptName).toBe("client-loader.js");

		const script = await fetch(`${relayHttpUrl()}/${scriptName}`);
		expect(script.status).toBe(200);
		expect(script.headers.get("cache-control")).toBe("public, max-age=300");
		expect(script.headers.get("content-type")).toMatch(/^text\/javascript/);
		expect((await fetch(`${relayHttpUrl()}/rooms.js`)).status).toBe(200);
		expect((await fetch(`${relayHttpUrl()}/rooms.css`)).status).toBe(200);

		const head = await fetch(`${relayHttpUrl()}/`, { method: "HEAD" });
		expect(head.status).toBe(200);
		expect(await head.text()).toBe("");
		expect((await fetch(`${relayHttpUrl()}/`, { method: "POST" })).status).toBe(
			404,
		);
	...
	it("rejects guests before a host creates the room", async () => {
		...
		const close = await waitEvent<CloseEvent>(
			guest,
			"close",
			"missing-room guest close",
		);
	...
	it("routes opaque envelopes between host and guest", async () => {
		...
		expect(JSON.parse(await waitText(host, "peer join"))).toEqual({
			t: "peer-joined",
			peer: 1,
		});
		...
		expect(unpackEnvelope(await broadcast)?.payload).toEqual(
			new Uint8Array([9]),
		);

		const targeted = waitBinary(guest, "targeted guest frame");
		host.send(packEnvelope(1, new Uint8Array([7])));
		expect(unpackEnvelope(await targeted)?.payload).toEqual(
			new Uint8Array([7]),
		);
	});
	...
	it("enforces one host and closes guests when the room host leaves", async () => {
		...
		const duplicateClose = await waitEvent<CloseEvent>(
			duplicateHost,
			"close",
			"duplicate host close",
		);
		expect(duplicateClose.code).toBe(4009);
		expect(duplicateClose.reason).toBe(
		...
		const guestClose = waitEvent<CloseEvent>(
			guest,
			"close",
			"guest room close",
		);
...
describe("omp-collab-relay hardening", () => {
	it("refuses non-loopback bind addresses and invalid caps", () => {
		expect(() => startRelay({ port: 0, quiet: true, bind: "0.0.0.0" })).toThrow(
			/non-loopback/,
		);
		expect(() =>
			startRelay({ port: 0, quiet: true, bind: "192.168.4.207" }),
		).toThrow(/non-loopback/);
		expect(() =>
			startRelay({ port: 0, quiet: true, maxFrameBytes: 16 * 1024 * 1024 + 1 }),
		).toThrow(/cannot exceed/);
		expect(() =>
			startRelay({ port: 0, quiet: true, webRoot: "relative" }),
		).toThrow(/existing absolute directory/);
	});
	...
	it("caps guests per room with retryable 4030", async () => {
		...
		expect(
			unpackEnvelope(await waitBinary(host, "post-reject envelope"))?.payload,
		).toEqual(new Uint8Array([42]));
	});

	it("hosts multiple rooms and exposes registered read-only links in the dashboard directory", async () => {
		start();
		const host = socket(`/r/${ROOM}?role=host`);
		const secondHost = socket(`/r/${OTHER_ROOM}?role=host`);
		await Promise.all([waitOpen(host), waitOpen(secondHost)]);

		const key = Buffer.alloc(32, 7).toString("base64url");
		const link = `https://omp.midsorbet.me/#${relay!.url}/r/${ROOM}.${key}`;
		const registration = await fetch(`${relayHttpUrl()}/api/rooms`, {
			method: "POST",
		...
		expect(await (await fetch(`${relayHttpUrl()}/api/rooms`)).json()).toEqual([
		...
		host.close(1000);
		await waitEvent<CloseEvent>(host, "close", "first host close");
		expect(await (await fetch(`${relayHttpUrl()}/api/rooms`)).json()).toEqual(
			[],
		);
	...
	it("closes a guest exceeding the frame cap with retryable 4013", async () => {
		...
		expect(JSON.parse(await waitText(host, "peer left"))).toEqual({
			t: "peer-left",
			peer: 1,
		});
	});
	...
	it("resets the room idle deadline on valid frame activity", async () => {
		...
		const hostClose = waitEvent<CloseEvent>(
			host,
			"close",
			"post-activity idle close",
			3_000,
		);
		expect((await hostClose).code).toBe(4001);
	});
