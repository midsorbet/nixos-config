...
- Only invoke `git commit`; do not invoke `git push` directly.
  - In `/Users/me/vault/projects/<name>`, the configured post-commit hook automatically commits the vault gitlink and, when the checks documented in `/Users/me/vault/AGENTS.md` pass, automatically pushes the project branch. It does not push the vault repository. Follow the vault rules and do not create a duplicate pointer commit.
