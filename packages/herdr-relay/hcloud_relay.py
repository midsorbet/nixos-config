...
def relay_command(arguments, *, timeout=120, capture=True, environment=None):
    ...
    try:
        return subprocess.run(
            arguments, check=True, timeout=timeout, text=True,
            capture_output=capture, env=environment,
        ).stdout
    except subprocess.CalledProcessError:
        raise RelayError(f"{Path(arguments[0]).name} failed; no credentials were logged") from None
    except subprocess.TimeoutExpired:
        raise RelayError(f"{Path(arguments[0]).name} timed out; check status for unfinished cloud operations") from None
...
class HcloudRelay:
    ...
    def run(self, *arguments, timeout=900):
        return relay_command([
            "hcloud", "--config", "/dev/null", "--endpoint", "https://api.hetzner.cloud/v1",
            "--http-timeout", "30s", *map(str, arguments),
        ], timeout=timeout, environment=self.environment)
    ...
    def owned_servers(self):
        return [server for server in self.resources("server") if is_relay_server(server)]

    def delete_server(self, server_id):
        """Delete only a freshly verified owned ID, then confirm its absence."""
        matches = [server for server in self.resources("server", None) if server["id"] == server_id]
        if not matches:
            return
        if len(matches) != 1 or not is_relay_server(matches[0]):
            raise RelayError(f"Refusing to delete server {server_id}: ownership does not match")
        self.run("server", "delete", server_id)
        for _ in range(5):
            if all(server["id"] != server_id for server in self.resources("server", None)):
                return
            time.sleep(1)
...
def is_relay_server(server):
    ...
    return expected_role is not None and labels.get("herdr-relay") == "hooh" and labels.get("role") == expected_role
...
def one_relay_resource(resources, description):
    ...
    if len(resources) != 1:
        raise RelayError(f"Expected exactly one {description}; run prepare or inspect project resources")
    return resources[0]


def relay_network(cloud):
    """Read existing resources; starting a session never provisions persistent infrastructure."""
    endpoint = one_relay_resource(cloud.resources("primary-ip", OWNER_SELECTOR + ",role=endpoint"), "retained IPv4")
    firewall = one_relay_resource(cloud.resources("firewall", OWNER_SELECTOR + ",role=firewall"), "relay firewall")
    if endpoint.get("type") != "ipv4" or endpoint.get("auto_delete") is not False:
        raise RelayError("Hooh must use a retained Primary IPv4")
    location = endpoint.get("location") or endpoint.get("datacenter", {}).get("location", {})
...
def refuse_existing_hooh(cloud):
    ...
    if any(server.get("name") in SERVER_ROLES for server in cloud.resources("server", None)):
        raise RelayError("Hooh or its image builder already exists; inspect status before creating another")
