...
from hcloud_relay import (
    HcloudRelay, OWNER_SELECTOR, SERVER_ROLES, RelayError, is_relay_server,
    refuse_existing_hooh, relay_command, relay_expiry, relay_network, write_relay_userdata,
)
...
def stop_frp(state):
    ...
    if sys.platform == "darwin":
        registered = subprocess.run(["launchctl", "print", relay_service()], capture_output=True).returncode == 0
        if registered:
...
def connect_frp(config, state):
    ...
    result = subprocess.run([
        "timeout", "--signal=TERM", "--kill-after=5s", str(remaining),
        "frpc", "-c", config["frpcConfig"],
    ]).returncode
...
def verify_mini_endpoint(config, address, expiry):
    ...
    expected = Path(config["miniHostPublicKeyFile"]).read_text().split()[:2]
    deadline = min(time.time() + 180, expiry)
    while time.time() < deadline:
        scan = subprocess.run([
            "ssh-keyscan", "-T", "3", "-t", "ed25519", "-p", "2222", address,
        ], capture_output=True, text=True, timeout=10)
        keys = [line.split()[1:3] for line in scan.stdout.splitlines() if not line.startswith("#")]
        if expected in keys:
            return
        time.sleep(2)
    raise RelayError("frp did not expose Mini's expected SSH host key before the readiness deadline")
...
def start_relay(config, state, cloud, seconds):
    ...
    snapshots = cloud.resources("image", OWNER_SELECTOR + ",role=relay-image,identity=" + config["imageIdentity"], "--type", "snapshot", "--architecture", "x86")
    ...
    addresses = {record[4][0] for record in socket.getaddrinfo(config["hostname"], 2222, socket.AF_INET, socket.SOCK_STREAM)}
    if addresses != {endpoint["ip"]}:
        raise RelayError(f"Set the DNS-only A record for {config['hostname']} to {endpoint['ip']} before starting")
    relay_command(["launchctl", "print", relay_service()])
    # Verify the local host-key prerequisite before creating anything billable.
    if not Path(config["miniHostPublicKeyFile"]).is_file():
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="start-", dir=state) as temporary:
            ...
            cloud.run(
                "server", "create", "--name", "hooh", "--type", "cpx11", "--location", "hil",
                "--image", snapshot["id"], "--primary-ipv4", endpoint["id"], "--without-ipv6",
                "--firewall", firewall["id"], "--label", OWNER_SELECTOR, "--label", "role=session",
                "--label", f"expires-at={expiry}", "--user-data-from-file", userdata,
            )
            server = next(server for server in cloud.owned_servers() if server["name"] == "hooh")
            new_lease = temporary / "lease.json"
            new_lease.write_text(json.dumps({"serverId": server["id"], "expiresAt": expiry}))
            ...
            ready = True
            return {"status": "ready", "serverId": server["id"], "hostname": config["hostname"], "port": 2222, "expiresAt": expiry}
    finally:
        if not ready:
            try:
                stop_frp(state)
            finally:
                # Reconcile ambiguous CLI failures, without ever retrying server creation.
    ...
    for server in cloud.resources("server"):
        ...
        try:
            if not is_relay_server(server):
                raise RelayError(f"Server {server['id']} has inconsistent ownership labels")
...
def relay_status(cloud):
    return {
        "servers": [{"id": server["id"], "name": server["name"], "status": server["status"], "expiresAt": relay_expiry(server)} for server in cloud.owned_servers()],
        "endpoints": [{"id": ip["id"], "ip": ip["ip"]} for ip in cloud.resources("primary-ip")],
        "snapshots": [{"id": image["id"], "created": image["created"]} for image in cloud.resources("image", OWNER_SELECTOR + ",role=relay-image", "--type", "snapshot")],
    }


def main():
    parser = argparse.ArgumentParser(prog="herdr-relay", description="Use hcloud and frp for the ephemeral Hooh relay")
    parser.add_argument("--config", default=os.environ.get("HERDR_RELAY_CONFIG", "/etc/herdr-relay.json"))
    commands = parser.add_subparsers(dest="command", required=True)
    commands.add_parser("prepare", help="create retained infrastructure and a NixOS snapshot").add_argument("--flake", required=True)
    commands.add_parser("start", help="create a leased VPS and start frp").add_argument("--ttl", type=parse_relay_ttl, default=28800)
    for command, help_text in [("stop", "stop frp and delete the session VPS"), ("status", "show cloud resources"), ("reap", "delete expired VPSs"), ("connect", "run frpc under launchd for the current lease")]:
    ...
    if state.is_symlink() or state.stat().st_uid != os.getuid():
        raise RelayError("Relay state directory must be owned by the current user and not a symlink")
    state.chmod(0o700)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("herdr-relay: interrupted; inspect status if cloud cleanup could not finish", file=sys.stderr)
        sys.exit(130)
