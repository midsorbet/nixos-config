...
7. `shopping_handoff_ready`: For Walmart, prefer a chatbot/Sparky prompt or a clean checklist over high-speed item-by-item automation. Stop at checkout, payment, reservation, CAPTCHA, or human-check gates. If Walmart is hostile or the user will shop manually, keep the grocery state in the vault or iCloud Groceries and reconcile from the receipt later.
